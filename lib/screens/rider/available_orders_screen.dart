...
class _AvailableOrdersScreenState extends State<AvailableOrdersScreen> {
  ...
  Widget _buildEarningsOverview(RiderProvider provider) {
    ...
    return Container(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          _buildEarningStat('TODAY', 'KSh ${earnings['total_earned'] ?? '0'}', isPrimary: true),
          _buildDivider(),
          _buildEarningStat('TRIPS', '${earnings['delivery_count'] ?? '0'}'),
          _buildDivider(),
          _buildEarningStat('TIPS', 'KSh ${earnings['total_tips'] ?? '0'}'),
        ],
      ),
    );
  }

  Widget _buildDivider() => Container(width: 1, height: 35, color: Colors.grey.shade100);

  Widget _buildEarningStat(String label, String value, {bool isPrimary = false}) {
    return Column(
      children: [
        ...
        Text(
          ...
          style: TextStyle(
            ...
            color: isPrimary ? AppTheme.primaryColor : const Color(0xFF1E293B)
          )
        ),
      ],
    );
      ...
      child: Column(
        children: [
          Container(
            ...
            child: Icon(Icons.wifi_off_rounded, size: 48, color: Colors.grey.shade300),
          ),
          const SizedBox(height: 24),
          const Text('You are Offline', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 20, color: AppTheme.primaryColor)),
          const SizedBox(height: 8),
          const Text(
            'Go online to see delivery requests near you and start earning.', 
            textAlign: TextAlign.center, 
            style: TextStyle(color: Colors.grey, fontWeight: FontWeight.w500, height: 1.4)
          ),
        ],
      ),
    );
  ...
  Widget _buildEmptyState() {
    return Container(
      ...
      child: Column(
        children: [
          Icon(Icons.radar_rounded, size: 64, color: AppTheme.primaryColor.withValues(alpha: 0.05)),
          const SizedBox(height: 20),
          const Text('Searching...', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 16, color: AppTheme.primaryColor)),
          const SizedBox(height: 8),
          const Text('New requests will appear here in real-time.', style: TextStyle(color: Colors.grey, fontWeight: FontWeight.w600)),
        ],
      ),
    );
  }
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                ...
                const SizedBox(height: 20),
                _buildRouteItem(Icons.storefront_rounded, 'PICKUP', order.storeName ?? 'Store Location', isStore: true),
                const Padding(
                  padding: EdgeInsets.only(left: 11),
                  child: Align(alignment: Alignment.centerLeft, child: Text('⋮', style: TextStyle(color: Colors.grey, fontSize: 18))),
                ),
                _buildRouteItem(Icons.location_on_rounded, 'DROP-OFF', order.addressString ?? 'Customer Address'),
                const SizedBox(height: 20),
                ...
                Row(
                  ...
                  children: [
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('POTENTIAL EARNING', style: TextStyle(color: Colors.grey, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 0.5)),
                        const SizedBox(height: 4),
                        Text('KSh ${order.deliveryFee.toStringAsFixed(0)}', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 24, color: AppTheme.primaryColor)),
                      ],
                    ),
                    SizedBox(
                      height: 50,
  ...
  Widget _buildRouteItem(IconData icon, String label, String address, {bool isStore = false}) {
    return Row(
      children: [
        ...
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(label, style: TextStyle(color: Colors.grey.shade400, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 0.5)),
              const SizedBox(height: 2),
              Text(address, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: Color(0xFF1E293B)), maxLines: 1, overflow: TextOverflow.ellipsis),
            ],
          ),
        ),
      ],
